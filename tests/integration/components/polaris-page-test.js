...
module('Integration | Component | polaris page', function(hooks) {
  ...
  test('it renders the page correctly', async function(assert) {
    ...
    const page = pages[0];
    assert.dom(page).hasNoClass('Polaris-Page--fullWidth', 'does not apply full width class');
    assert.dom(page).hasNoClass('Polaris-Page--singleColumn', 'does not apply single column class');

    const headers = findAll(headerSelector);
    assert.equal(headers.length, 1, 'renders one page header div');
    assert.dom(headers[0]).hasNoClass(
    ...
    assert.dom(page).hasClass('Polaris-Page--fullWidth', 'fullWidth set - applies fullWidth class');
  ...
  test('it handles primary action correctly when supplied', async function(assert) {
    ...
    assert.dom(primaryButton).hasNoClass(
    ...
    assert.dom(primaryButton).hasClass('Polaris-Button--loading', 'primary action button goes into loading state');
  ...
  test('it handles secondary actions correctly when supplied', async function(assert) {
    ...
    assert.dom(secondaryButtons[0]).hasText(
      'First secondary action',
      'first secondary action - uses correct text on button'
    );
    assert.dom(secondaryButtons[1]).hasText(
    ...
    return settled()
      .then(() => {
        ...
        assert.dom(focussedSecondaryButtonSelector).doesNotExist('no focussed buttons after clicking first secondary action');
      ...
      .then(() => {
        ...
        assert.dom(focussedSecondaryButtonSelector).doesNotExist('no focussed buttons after clicking second secondary action');
      });
  });
  ...
  test('it handles breadcrumbs correctly', async function(assert) {
    await render(hbs`{{polaris-page title="This is a page" breadcrumbs=breadcrumbs}}`);
    ...
    assert.dom(contents[0]).hasText('No, really!', 'breadcrumb renders text from last breadcrumb in list');
  ...
  test('it allows the primary action to be rendered as not primary', async function(assert) {
    ...
    assert.dom(primaryButton).hasClass('Polaris-Button--primary', 'primary true - button is rendered as primary');

    this.set('shouldRenderPrimaryActionAsPrimary', undefined);
    assert.dom(primaryButton).hasClass(
      'Polaris-Button--primary',
